#!/usr/bin/env node
/**
 * Builds public/Animation/Coupon.dark.json from Coupon.json by re-rendering
 * embedded gift + dollar PNGs from public/Icon/gift/*.svg (Puppeteer → WebP),
 * same dark treatment as gift SVG → WebP in other theme scripts. Ticket/coupon PNG rasters unchanged
 * (no vector sources in repo).
 *
 * Usage: npm run generate:coupon-dark-lottie
 */
...
/** Dark theme: interior panels dim, gift outline/fill pure white (#fff). */
...
/** Map Lottie asset id → source SVG + export size. `null` = leave embedded raster as in Coupon.json. */
...
async function main() {
  const srcPath = join(animDir, "Coupon.json");
  const lottie = JSON.parse(readFileSync(srcPath, "utf8"));
  if (!Array.isArray(lottie.assets)) {
    throw new Error("Coupon.json: missing assets[]");
  }
  ...
  try {
    ...
    if (n !== 3) {
      throw new Error(`Expected 3 SVG-backed assets (gift cover, giftbox, dollar); re-rendered ${n}`);
    }
  } finally {
    await browser.close();
  }

  lottie.meta = { ...(lottie.meta || {}), g: "generate-coupon-dark-lottie.mjs", theme: "dark" };

  writeFileSync(join(animDir, "Coupon.dark.json"), JSON.stringify(lottie));
  console.log(`Wrote Coupon.dark.json (gift + dollar → WebP dark; ticket rasters unchanged)`);
}
